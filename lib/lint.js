function declarationSupportedValues(declarationName) {
	switch (declarationName) {
		...
		case 'max_line_length': return ['#', 'off'];
		default: return null;
	}
}
					...
					if (abstractDeclaration[0] === 'root') {
						...
						if (sawAHeader) {
							result.push({
								...
								excerpt: 'Declaration line \'root\' must be before all header lines',
						...
						if (!['true', 'false'].includes(abstractDeclaration[1])) {
							result.push({
								...
								description: 'The only valid values for \'root\' declarations are `true` and `false`.',
								url: 'https://editorconfig-specification.readthedocs.io/en/latest/#file-processing',
								solutions: []
							});
						}
						...
						if (!sawAHeader) {
							result.push({
								...
								excerpt: 'Only \'root\' declarations can be present within the file preamble',
